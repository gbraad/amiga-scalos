...
static void BlitTrans(struct DragHandle *dh, 
	...
	LONG SrcX, LONG SrcY, LONG DestX, LONG DestY, 
	LONG Width, LONG Height, APTR Mask);
	...
	LONG x0, LONG y0)
{
	...
	if (MaskBMSelected)
		...
		bm.BytesPerRow = ((ggWidth + 15) & ~0xf) / 8;
		bm.Rows = gg->Height;
		bm.Flags = 0;
		bm.Depth = 1;
		bm.pad = 0;
		memset(bm.Planes, 0, sizeof(bm.Planes));

		bm.Planes[0] = AllocPlane = AllocVec(bm.BytesPerRow * bm.Rows, MEMF_PUBLIC | ChipMemAttr() | MEMF_CLEAR);
		d1(kprintf("%s/%s/%ld: Planes[0]=%08lx  Size=%ld\n", \
			__FILE__, __FUNC__, __LINE__, bm.Planes[0], bm.BytesPerRow * bm.Rows));
		if (bm.Planes[0])
			{
			...
			BltBitMap(MaskBMSelected, 0, 0, &bm,
				LeftSpace, TopSpace,
				Width, Height,
				ABC | ABNC, ~0, NULL);

			MaskPlane = bm.Planes[0];
	...
	LONG XOffset, LONG YOffset)
{
	...
	do	{
		...
		tempBM.BytesPerRow = ((bob->scbob_width + 15) & ~15) / 8;
		tempBM.Rows = bob->scbob_height;
		tempBM.Flags = 0;
		tempBM.pad = 0;
		tempBM.Depth = 1;

		d1(kprintf("%s/%s/%ld: BytesPerRow=%ld  Rows=%ld\n", __FILE__, __FUNC__, __LINE__, tempBM.BytesPerRow, tempBM.Rows));

		memset(tempBM.Planes, 0, sizeof(tempBM.Planes));
		tempBM.Planes[0] = AllocVec(tempBM.BytesPerRow * tempBM.Rows, MEMF_PUBLIC | ChipMemAttr());
		if (NULL == tempBM.Planes[0])
			break;

		xMax = (bob->scbob_width + 15) / 16;
		pw1 = (UWORD *) tempBM.Planes[0];
		pwm = (UWORD *) Mask;
...
#if 0
		if (Mask)
			CopyMem(Mask, tempBM.Planes[0], tempBM.BytesPerRow * tempBM.Rows);
		else
			memset(tempBM.Planes[0], 0xff, tempBM.BytesPerRow * tempBM.Rows);
#endif
		...
		if (IsBobPosition(bob->scbob_Bob.scbob_Bob2.scbob2_xpos)
			...
			if (SizeX < bob->scbob_width || SizeY < bob->scbob_height)
				...
				BlitTrans(dh, bob, bob->scbob_Bob.scbob_Bob2.scbob2_bitmap,
					...
					0, 0,
					bob->scbob_width, bob->scbob_height,
					bob->scbob_Bob.scbob_Bob2.scbob2_shadow->Planes[0]
					);
		...
		for (bob=dh->drgh_boblist; bob; bob = (struct ScaBob *) bob->scbob_Node.mln_Succ)
			...
			if (AlphaSrcPtr)
				...
				BlitARGBMaskAlpha(bob->scbob_width, bob->scbob_height,
					...
					bob->scbob_Bob.scbob_Bob2.scbob2_shadow1->Planes[0], ALPHA_OPAQUE,
					AlphaSrcPtr,
					bob->scbob_LeftBorder, bob->scbob_BoundsWidth);
				}
			else
		...
		const UBYTE *Mask, ULONG Trans)
{
	...
	MaskBytesPerRow = ((SrcWidth + 15) & ~0x0f) / 8;
...
{
	ULONG y;
	ULONG MaskBytesPerRow = ((SrcWidth + 15) & ~0x0f) / 8;
