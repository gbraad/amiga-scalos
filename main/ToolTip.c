...
static void DrawRGBPixelWeighted(struct RastPort *rp,
...
static void DrawWuLine(struct RastPort *rp, int X1, int Y1, UWORD LinePen)
{
	...
	Color32 = COMBINE_RGB(rl, gl, bl);

	d1(KPrintF("%s/%s/%ld: LinePen=%ld  r=%ld g=%ld b=%ld  Color32=%08lx\n", \
		__FILE__, __FUNC__, __LINE__, LinePen, rl, gl, bl, Color32));
...
//			  ASSERT( ( Weighting ^ 255 ) < 256 );
			...
			rr = ( rb > rl ? ( ( UBYTE )( ( ( double )( grayl<grayb?Weighting:
			          (Weighting ^ 255)) ) / 255.0 * ( rb - rl ) + rl ) ) :
				  ( ( UBYTE )( ( ( double )( grayl<grayb?Weighting:(Weighting ^ 255)) )
			          / 255.0 * ( rl - rb ) + rb ) ) );
			gr = ( gb > gl ? ( ( UBYTE )( ( ( double )( grayl<grayb?Weighting:
			          (Weighting ^ 255)) ) / 255.0 * ( gb - gl ) + gl ) ) :
				  ( ( UBYTE )( ( ( double )( grayl<grayb?Weighting:(Weighting ^ 255)) )
			          / 255.0 * ( gl - gb ) + gb ) ) );
			br = ( bb > bl ? ( ( UBYTE )( ( ( double )( grayl<grayb?Weighting:
			...
			rr = ( rb > rl ? ( ( UBYTE )( ( ( double )( grayl<grayb?(Weighting ^ 255):
			    Weighting) ) / 255.0 * ( rb - rl ) + rl ) ) :
			    ( ( UBYTE )( ( ( double )( grayl<grayb?(Weighting ^ 255):Weighting) )
			    / 255.0 * ( rl - rb ) + rb ) ) );
			gr = ( gb > gl ? ( ( UBYTE )( ( ( double )( grayl<grayb?(Weighting ^ 255):
			    Weighting) ) / 255.0 * ( gb - gl ) + gl ) ) :
			    ( ( UBYTE )( ( ( double )( grayl<grayb?(Weighting ^ 255):Weighting) )
			    / 255.0 * ( gl - gb ) + gb ) ) );
			br = ( bb > bl ? ( ( UBYTE )( ( ( double )( grayl<grayb?(Weighting ^ 255):
	...
	WORD Segment, UBYTE color1, UBYTE color2)
{
	...
	for (i = 1; i <= dxt; i++)
		...
		xx = xc2 - x;
		if (Segment & ELLIPSE_SEGMENT_TOPLEFT)
			DrawRGBPixelWeighted(rp, x, y, &rgbColor2, iweight);	// octand 8
		if (Segment & ELLIPSE_SEGMENT_TOPRIGHT)
			DrawRGBPixelWeighted(rp, xx, y, &rgbColor1, iweight);	// octand 1

		if (Segment & ELLIPSE_SEGMENT_TOPLEFT)
			DrawRGBPixelWeighted(rp, x, ys, &rgbColor2, weight);	// octand 8
		if (Segment & ELLIPSE_SEGMENT_TOPRIGHT)
			DrawRGBPixelWeighted(rp, xx, ys, &rgbColor1, weight);	// octand 1

		/* Lower half */
		yy = yc2 - y;
		if (Segment & ELLIPSE_SEGMENT_BOTTOMLEFT)
			DrawRGBPixelWeighted(rp, x, yy, &rgbColor1, iweight);	// octand 5
		if (Segment & ELLIPSE_SEGMENT_BOTTOMRIGHT)
			DrawRGBPixelWeighted(rp, xx, yy, &rgbColor2, iweight);	// octand 4

		yy = yc2 - ys;
		if (Segment & ELLIPSE_SEGMENT_BOTTOMLEFT)
			DrawRGBPixelWeighted(rp, x, yy, &rgbColor1, weight);	// octand 5
		if (Segment & ELLIPSE_SEGMENT_BOTTOMRIGHT)
			DrawRGBPixelWeighted(rp, xx, yy, &rgbColor2, weight);	// octand 4
		}
	...
	for (i = 1; i <= dyt; i++)
		...
		xx = xc2 - x;
		if (Segment & ELLIPSE_SEGMENT_TOPLEFT)
			DrawRGBPixelWeighted(rp, x, y, &rgbColor1, iweight);	// octand 7
		if (Segment & ELLIPSE_SEGMENT_TOPRIGHT)
			DrawRGBPixelWeighted(rp, xx, y, &rgbColor2, iweight);	// octand 2

		if (Segment & ELLIPSE_SEGMENT_BOTTOMLEFT)
			DrawRGBPixelWeighted(rp, x, yy, &rgbColor2, iweight);	// octand 6
		if (Segment & ELLIPSE_SEGMENT_BOTTOMRIGHT)
			DrawRGBPixelWeighted(rp, xx, yy, &rgbColor1, iweight); 	// octand 3

		/* Right half */
		xx = 2 * xc - xs;
		if (Segment & ELLIPSE_SEGMENT_TOPLEFT)
			DrawRGBPixelWeighted(rp, xs, y, &rgbColor1, weight); 	// octand 7
		if (Segment & ELLIPSE_SEGMENT_TOPRIGHT)
			DrawRGBPixelWeighted(rp, xx, y, &rgbColor2, weight);   	// octand 2?

		if (Segment & ELLIPSE_SEGMENT_BOTTOMLEFT)
			DrawRGBPixelWeighted(rp, xs, yy, &rgbColor2, weight);	// octand 6?
		if (Segment & ELLIPSE_SEGMENT_BOTTOMRIGHT)
			DrawRGBPixelWeighted(rp, xx, yy, &rgbColor1, weight);	// octand 3?
		}
}
