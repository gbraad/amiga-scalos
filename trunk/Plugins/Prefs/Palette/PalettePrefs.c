		...
		while (RETURN_OK == Result)
			...
			if (ID_PENS == cn->cn_ID)
				...
				for (n=0; n<ScreenColorListEntries; n++)
					...
					sple.sple_Index = n;
					sple.sple_Red = splc.splc_Red;
					sple.sple_Green = splc.splc_Green;
					sple.sple_Blue = splc.splc_Blue;
...
static LONG WritePrefsFile(struct PalettePrefsInst *inst, CONST_STRPTR Filename)
{
	...
	do	{
		...
		while (RETURN_OK == Result)
			...
			splc.splc_Red = sple->sple_Red;
			splc.splc_Green = sple->sple_Green;
			splc.splc_Blue = sple->sple_Blue;
		...
		while (RETURN_OK == Result)
			...
			sPen.sp_pentype = snle->snle_PenIndex;
			sPen.sp_pen = snle->snle_PenNr;
