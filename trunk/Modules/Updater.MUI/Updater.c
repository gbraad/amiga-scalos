...
struct FileHandler
	{
	APTR (*hfh_Open)(CONST_STRPTR Filename, size_t TotalLength);
	size_t (*hfh_Write)(APTR Handle, CONST_STRPTR Data, size_t DataLength);
	void (*hfh_Close)(APTR Handle);
	};
...
static Object *GaugeProgressMsg;
		...
		SubWindow, WIN_Main = WindowObject,
			...
			WindowContents, VGroup,
				...
				Child, HGroup,
					...
					Child, GaugeProgressMsg = GaugeObject,
						GaugeFrame,
...
static void ProcessVersionFile(CONST_STRPTR text, size_t len)
{
	...
	while (len)
		...
		if (LineLen && ';' != *text && '#' != *text)
			{
			enum { ARG_DIR, ARG_FILE, ARG_VERSION, ARG_PKG };
			static CONST_STRPTR Template = "DIR/A/K,FILE/A/K,VERSION/A/K,PKG/K";
			...
			if (ReadArgs(Template, Args, &ReadArgs))
				...
				d1(KPrintF("%s/%s/%ld: DIR=<%s> FILE=<%s> VERSION=<%s> PKG=<%s>\n", __FILE__, __FUNC__, __LINE__, \
					Args[ARG_DIR], Args[ARG_FILE], Args[ARG_VERSION], Args[ARG_PKG]));
...
static ULONG ComponentsDisplayHookFunc(struct Hook *hook, Object *obj, struct NList_DisplayMessage *msg)
{
	if (msg->entry)
		...
		msg->strings[3] = GetLocString(MSGID_COMPONENTLIST_AVAILABLE_VERSION);
...
static void CheckForUpdatesHookFunc(struct Hook *hook, Object *obj, Msg *msg)
{
	...
	if (curl)
		...
		set(GaugeProgressMsg, MUIA_Gauge_InfoText, TextProgressBuffer);
...
//		curl_easy_setopt(curl, CURLOPT_PROXYUSERPWD, "user:password");
		...
		if (0 == res)
			{
			set(GaugeProgressMsg, MUIA_Gauge_InfoText, GetLocString(MSG_PROGRESS_PROCESS_HTTP_RESPONSE));
	...
	set(GaugeProgressMsg, MUIA_Gauge_InfoText, GetLocString(Success ? MSG_PROGRESS_DONE : MSG_PROGRESS_FAILED));
...
static void StartUpdateUpdateHookFunc(struct Hook *hook, Object *obj, Msg *msg)
{
	...
	set(GaugeProgressMsg, MUIA_Gauge_InfoText, GetLocString(MSG_PROGRESS_START_UPDATE));
	set(GaugeProgressMsg, MUIA_Gauge_Current, 0);
	...
	if (curl)
		...
		for (n = 0; n < Entries; n++)
			...
			do	{
				...
				set(GaugeProgressMsg, MUIA_Gauge_Current, (100 * Count) / SelectedCount);
				...
				free(Buffer);
				len = 1 + strlen(ScalosHttpAddr) + strlen(cle->cle_Package);
				Buffer = malloc(len);
				if (NULL == Buffer)
					break;
				...
				set(GaugeProgressMsg, MUIA_Gauge_InfoText, GetLocString(MSG_PROGRESS_DONE));

				snprintf(Buffer, len, "%s%s", ScalosHttpAddr, cle->cle_Package);
...
//				curl_easy_setopt(curl, CURLOPT_PROXYUSERPWD, "user:password");
	...
	set(GaugeProgressMsg, MUIA_Gauge_Current, 100);
	set(GaugeProgressMsg, MUIA_Gauge_InfoText, GetLocString(MSG_PROGRESS_DONE));
}
...
static int CheckForUpdatesProgressFunc(void *clientp, double dltotal, double dlnow, double ultotal, double ulnow)
{
	LONG dlLevel = (LONG) ((100 * dlnow) / dltotal);

	d2(kprintf("%s/%ld: dlLevel=%ld\n", __FUNC__, __LINE__, dlLevel));
	set(GaugeProgressMsg, MUIA_Gauge_Current, dlLevel);
