...
SAVEDS(void) INTERRUPT ProcRunnerTask(void)
{
	...
	if (ID_IMSG == msg->ScalosMessage.sm_Signature)
		...
		case MTYP_RunProcess:
			// make sure there is no problem with backward-compatibility
			iwt = (struct internalScaWindowTask *) msg->WindowTask;

			d1(kprintf("%s/%s/%ld: msg=%08lx  iwt=%08lx\n", __FILE__, __FUNC__, __LINE__, msg, iwt));

			if (iwt)
				{
				ScalosObtainSemaphoreShared(&iwt->iwt_ChildProcessSemaphore);

				d1(kprintf("%s/%s/%ld: mlh_Head=%08lx  mlh_Tail=%08lx  mlh_TailPred=%08lx\n", __FILE__, __FUNC__, __LINE__, \
					iwt->iwt_ChildProcessSemaphore.ss_WaitQueue.mlh_Head,\
					iwt->iwt_ChildProcessSemaphore.ss_WaitQueue.mlh_Tail, \
					iwt->iwt_ChildProcessSemaphore.ss_WaitQueue.mlh_TailPred));
				}
		...
		case MTYP_AsyncBackFill:
			...
			iwt = (struct internalScaWindowTask *) smab->smab_WindowTask;
			if (iwt)
				{
